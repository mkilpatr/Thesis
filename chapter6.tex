...
\end{equation}
where $r$ is the POI which is unconstrained only by the observed data, $s_i(\overrightarrow{\theta})$ and $b_i(\overrightarrow{\theta})$ are nuisance parameters which have external constraints. Each of these three parameters are jointly fitted to get a value of $r$. 
...
\subsection{The Fundamental Particles}
 ...
 All matter can be explained by three kinds of elementary particles: leptons, quarks, and gauge bosons. Each of these can be distinguished by various quantum properties. The leptons and quarks are fermions, which are particles that have half-integer spin. Charged leptons are particles that only interact with the EM and Weak force, while neutral leptons, neutrinos, only interact weakly. Quarks interact with all three forces of the SM. The gauge bosons are the force carriers for each respective force and have integer spin. 
...
\end{figure}
 ...
 The interactions of all these particle fields are described by quantized fields whose operators describe the creation and annihilation of particles. Each of the forces of the SM have a corresponding gauge boson which is described by a quantized gauge field, see Fig. \ref{SMParticles}. The most well-known bosonic field is the EM field and its interactions. In order to write a concise theory of the particles in the SM, the symmetry and conservation laws of the SM can be derived by starting with Noether's Theorem.
 
 \subsection{Noether's Theorem}
 
 Noether's theorem states,"to each symmetry of a local Lagrangian, there corresponds a conserved current" \cite{peskin_introduction_1995}. This can be done by allowing for an infinitesimal symmetry variation. Requiring the Lagrangian to be invariant under $\phi(x)\rightarrow\phi^\prime(x)=\phi(x)+\alpha\Delta\phi(x)$, where $x$ is a shorthand for $(t,x,y,z)$, $\alpha$ is infinitesimal real parameter and $\Delta\phi$ is a deformation to the field, up to a 4-divergence, the Lagrangian transforms as,
 \begin{equation}\label{LagrangeFluctuation}
 \mathcal{L}(x)\rightarrow\mathcal{L}(x)+\alpha\partial_\mu\mathcal{J}^\mu(x),
 \end{equation}
 where $\mathcal{J}^\mu$ is a current. We then apply the Euler-Lagrange equation to Eqn. \ref{LagrangeFluctuation},
 \begin{equation}
 \partial_\mu(\frac{\partial\mathcal{L}}{\partial(\partial_\mu\phi)})-\frac{\partial\mathcal{L}}{\partial\phi}=0,
 \end{equation}
 where $\partial_\mu = (\partial_t, \partial_x, \partial_y, \partial_z)$ is the 4-divergence ($\mu=0,1,2,3$) with the addition of the fluctuation of the particle field. After some simplification we get a conserved current \cite{halzen_quarks_1984, peskin_introduction_1995},
 \begin{equation}
 \begin{split}
 ...
 \end{split},
 \end{equation} 
 where $\partial_\mu j^\mu(x) = \partial_0 j^0(x) - (\partial_1 j^1(x)+\partial_2 j^2(x)+\partial_3 j^3(x))$.
 We see from the above equation that the current $j^\mu(x)$ of the Lagrangian is conserved. Now let's apply this to the particle fields of the SM \cite{halzen_quarks_1984, peskin_introduction_1995}.
...
To allow for the field to be invariant, we must include a derivative, $D_\mu$, that is covariant under phase transformations, such that the derivative preserves the physical properties under the transformation,
 ...
 \end{equation}
 So after requiring that there be a local gauge transformation, we were forced to introduce a vector field $A_\mu$, called the gauge field, which couples to Dirac particles in the same way as the photon field. We will think of this new field as the real photon field, which means we need to add a kinetic energy portion to the Lagrangian since this is describing a real particle. This kinetic term will be invariant under Eqn. \ref{PhotonField}, which leads us to the final representation of the QED Lagrangian which can be written down concisely as, 
...
\end{equation}
where $A_{\mu}$ is the EM field operator and $F^{\mu\nu}$ is the EM field tensor. This Lagrangian describes the interactions between spin-$1/2$ charged particles and the $U(1)$ EM force. Each of the parts of this equation is Lorentz invariant which allows this to be true in all reference frames. We also find that the conserved current of QED is the electric charge, $Q=\int d^3xj^0$, which is the time varying current, $j^0$, integrated over all of space \cite{halzen_quarks_1984, peskin_introduction_1995}. 
...
\end{equation}
where $U$ is a $3\times3$ unitary matrix, $T_a$ with $a=1,\ldots,8$ are a set of linearly independent traceless $3\times3$ matrices, and $\alpha_a$ are the group parameters. Since $T_a$ does not necessarily commute with one another, we can see that it is a non-Abelian transformation and the commutator can be represented as,
\begin{equation}
[T_a, T_b]=if_{abc}T_c,
\end{equation}
...
The \W{} boson interacts very interestingly for quarks in the SM. Due to the $V-A$ nature of the weak interaction there is a mixing of quark flavors. They will mix the down-type particles between all three flavors \cite{kobayashi_cp-violation_1973}. The interactions for the generalized three generations of quarks is known as the Cabibbo-Kobayashi-Maskawa (CKM) matrix \cite{halzen_quarks_1984, griffiths_introduction_2008},
...
\end{split}
\end{equation}
where the transformation $e^{i\frac{\tau_a}{2}\alpha^a(x)}$ with $a = 1, 2, 3$ is the $SU(2)_L$ transformation and only acts on the left-handed doublet. The $SU(2)_L$ symmetry has 3 generators for the group which are the three $\tau_a$. The next step is to require that the Lagrangian is invariant under local $SU(2)_L\times U(1)_Y$. We allow for the following covariant derivatives \cite{halzen_quarks_1984},
\begin{equation}
\begin{split}
...
\begin{figure}
	...
	\includegraphics[width=0.5\textwidth]{Hierarchy.png}
 	\caption[Hierarchy Problem Loop Correction]{The loop corrections to the Higgs boson interacting with fermion and its superpartner. This is a next-to-leading order (NLO) correction to the Higgs boson mass.}
 	\label{HiggsMass} 
\end{figure}
...
\subsection{Mass Spectra}\label{subsec:StopMassSpec}
...
Here we see results from searches for the top squark from CMS, A Toroidal LHC Apparatus (ATLAS), and combined results from the Large Electron-Positron Collider (LEP). These have been completed with data from 2016 with $35.9 \text{ fb}^{-1}$, Run 2 with \datalumi, and $2.7 \text{ fb}^{-1}$, respectively. There are similar searches using data from 2016 in Ref. \cite{atlas_collaboration_search_2017, cms_collaboration_search_2017, sirunyan_search_2017-1, sirunyan_search_2017, cms_collaboration_search_2017-1}

\begin{figure}
\centering
	\includegraphics[width=0.45\textwidth]{LEP_stopLimits.png}
 	\caption[T2tt Limits from LEP]{Limits for the mass parameter space for T2tt decays using results from all the analysis in LEP including Aleph, Delphi, Opal, and L3 searches and exlusions provided by the CDF collaboration. With a current limit of 80 GeV to 120 GeV for a minimal neutralino mass \cite{lipniacka_understanding_2002}.}
 	\label{T2ttLEPAll} 
\end{figure}

\begin{figure}
\centering
	\includegraphics[width=0.50\textwidth]{T2tt_limits_summary_cms.png}
 	\caption[T2tt Limits from CMS]{Limits for the mass parameter space for T2tt decays using results from all the analysis in CMS. With a current limit of 1.05 TeV to 1.2 TeV for a minimal neutralino mass \cite{noauthor_https://twiki.cern.ch/twiki/bin/view/cmspublic/physicsresultssus_nodate}.}
 	\label{T2ttCMSAll} 
\end{figure}

\begin{figure}
\centering
	\includegraphics[width=0.60\textwidth]{ATLAS_topsquark_results.pdf}
 	\caption[T2tt Limits from ATLAS]{Limits for the mass parameter space for top squark decays using results from all the analysis in ATLAS. With a current limit of 700 GeV to 1.0 TeV for a minimal neutralino mass \cite{the_atlas_collaboration_susy_2019}.}
 	\label{T2ttATLASAll} 
\end{figure}


Searches for SUSY have been performed at many different detectors. In Figures \ref{T2ttLEPAll}, \ref{T2ttCMSAll}, \ref{T2ttATLASAll}, we have the 95\% confidence level exclusion limits for LEP including Aleph, Delphi, Opal, L3, and the CDF collaboration \cite{lipniacka_understanding_2002}, CMS detector \cite{noauthor_https://twiki.cern.ch/twiki/bin/view/cmspublic/physicsresultssus_nodate}, and ATLAS detector \cite{the_atlas_collaboration_susy_2019} at the LHC. The results from LEP are only considering the top squark decaying to a charm quark and neutralino and assuming a 100\% branching fraction with limits on the top squark at 80 GeV to 120 GeV. The results from CMS are including analysis on \datalumi{} for a few all-hadronic and leptonic decay modes of the top squark with limits of 1.05 TeV to 1.2 TeV for minimal neutralino mass. Another experiment at the LHC, called ATLAS, has performed searches for the top squark with limits on the mass as 700 GeV to 1.0 TeV at minimal neutralino mass using a combination of either 36.1 $\text{fb}^{-1}$ to \datalumi. 

From the Figures \ref{T2ttCMSAll} and \ref{T2ttATLASAll} \cite{noauthor_https://twiki.cern.ch/twiki/bin/view/cmspublic/physicsresultssus_nodate, the_atlas_collaboration_susy_2019}, we know that we are able to exclude a large mass range for the \st{} and \neutralino{}. Since this is for a range of luminosities, $36.1 \text{ fb}^{-1}$ to $139$ \fb, we can expect improved limits with all of the data from Run 2, which is \datalumi. The new version of the analysis also has a redesigned search region to allow for more sensitive results, while also improving various object definitions.
